...
contract Pair{
    ...
    modifier hasBalanceAndAllowance(address _token0Address, address _token1Address, uint256 _token0Amount, uint256 _token1Amount){
        erc20 token0Address = erc20(_token0Address);
        erc20 token1Address = erc20(_token1Address);  

        require(token0Address.balanceOf(msg.sender)>= _token0Amount,"Insufficient Balance");
        require(token1Address.balanceOf(msg.sender)>= _token1Amount,"Insufficient Balance");
        require(token0Address.allowance(msg.sender,address(this))== _token0Amount,"Insufficient allowance for token0");
        require(token1Address.allowance(msg.sender,address(this))==_token1Amount,"Insufficient allowance for token1");
    ...
    function createPair(address _token0Address, address _token1Address, uint256 _token0Amount, uint256 _token1Amount ) validTokenAddresses(_token0Address,  _token1Address) hasBalanceAndAllowance( _token0Address,  _token1Address,  _token0Amount,  _token1Amount)  public {
